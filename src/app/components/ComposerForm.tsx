...
const defaultCategories = ["학습팁", "시험대비", "단어", "후기", "질문", "자유"];
    ...
    <div className="min-h-screen bg-background flex flex-col">
            ...
            {categories.map((category) => (
              <Badge
                ...
                className={`cursor-pointer px-4 py-2 ${
                  selectedCategory === category ? "bg-primary text-white hover:bg-primary/90" : "hover:bg-muted"
                }`}
                onClick={() => {
      ...
      <div className="bg-white border-t border-border px-6 py-4">
        <div className="flex items-center gap-4">
          ...
          {selectedImages.length > 0 && <span className="ml-auto text-xs text-muted-foreground">{selectedImages.length} / 5</span>}
        </div>
      </div>
    </div>
  );
