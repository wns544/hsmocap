...
import { X, Image, Smile, Hash, ChevronDown } from "lucide-react";
...
export default function CreatePost() {
  ...
  const [selectedCategory, setSelectedCategory] = useState("학습팁");
    ...
    for (let i = 0; i < Math.min(files.length, 5 - selectedImages.length); i++) {
      ...
      reader.onload = (event) => {
        if (event.target?.result) {
          ...
          if (newImages.length === Math.min(files.length, 5 - selectedImages.length)) {
